...
<body>
	<center>
		...
		<table border="3">
			...
			<c:forEach items="${ANALYSIS_IT}" var="vo">
				<tr>
					<td>${vo.date}</td>
					<td>${vo.price}</td>
					<c:if test="${vo.updn2 == 1}">
						<td style="color: blue;">
						<img src="http://localhost:8081/phn/resources/img/ico_down.gif">
						${vo.yst}</td>
					</c:if>
					<c:if test="${vo.updn2 == 2}">
						<td style="color: Red;">
						<img src="http://localhost:8081/phn/resources/img/ico_up.gif">
						${vo.yst}</td>
					</c:if>
...
<img src="http://localhost:8081/phn/resources/img/money.png">
		<table border="1" style="visibility: hidden;">
			...
			<c:forEach items="${GRAPH_IT}" var="vo" varStatus="s">
				<tr>
					<td>${vo.name}</td>
