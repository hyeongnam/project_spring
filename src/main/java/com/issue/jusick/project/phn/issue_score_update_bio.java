...
public class issue_score_update_bio {
...
//		}
		...
		// IBM Watson 인스턴스 생성
		NaturalLanguageUnderstanding service = new NaturalLanguageUnderstanding("201"
				+ "7-02-27");
		...
		issue_data.add("팜스웰바이오");
		issue_data.add("이수앱지스");
		issue_data.add("셀트리온");
		issue_data.add("한미약품");
		issue_data.add("에이프로젠제약");
		issue_data.add("제넥신");
		issue_data.add("한올바이오파마");
		for (int i = 0; i < issue_data.size(); i++) {
			...
			String SQL = " update bio set issue= '" + issue + "'  " + " where name= '" + name + "' "
					+ " and date = '" + date + "'";
