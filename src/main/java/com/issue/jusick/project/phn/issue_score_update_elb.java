...
public class issue_score_update_elb {
...
//		}
		...
		issue_data.add("상신이디피");
		issue_data.add("세방전지");
		issue_data.add("이랜텍");
		issue_data.add("파워로직스");
		issue_data.add("SKC");
		issue_data.add("아트라스BX");
		issue_data.add("한화케미칼");
		issue_data.add("포스코케미칼");
		issue_data.add("솔브레인");
		issue_data.add("에코프로");
		issue_data.add("이엔에프테크놀로지");
		issue_data.add("엘앤에프");
		issue_data.add("피앤이솔루션");
		for (int i = 0; i < issue_data.size(); i++) {
			...
			String SQL = " update elb set issue= '" + issue + "'  " + " where name= '" + name + "' "
					+ " and date = '" + date + "'";
